...
Процедура ИсполняемыеСценарии() Экспорт
    ...
    ЮТТесты
		...
		.ДобавитьТестовыйНабор("ОбщиеТесты")
			...
			.ДобавитьТест("Предупреждение_depricated")
		...
		.ДобавитьТестовыйНабор("ВалидацияЧисел")
			...
			.ДобавитьТест("Успешно_ЧислоРавноМинимуму")
			.ДобавитьТест("Успешно_ЧислоРавноМаксимуму")
			...
			.ДобавитьТест("Успешно_ФорматЦелогоЧисла")
				.СПараметрами("int32")
				.СПараметрами("int64")
			.ДобавитьТест("Ошибка_НекорректныйФорматЦелого")
				.СПараметрами("int32", -3000000000)
				.СПараметрами("int32", +3000000000)
				.СПараметрами("int64", -10000000000000000000)
				.СПараметрами("int64", +10000000000000000000)
...
Процедура Ошибка_НетОбязательногоСвойства() Экспорт
	...
	ИмяСхемы = "object";
	МассивОшибок = Валидировать(ОбъектПроверки, ИмяСхемы).Ошибки;
...
Процедура Предупреждение_depricated() Экспорт
	ОбъектПроверки = "hello world!";
	ИмяСхемы = "deprecated";
	РезультатПроверки = Валидировать(ОбъектПроверки, ИмяСхемы);
	Предупреждения = РезультатПроверки.Предупреждения;
	Ошибки = РезультатПроверки.Ошибки;
	
	ЮТест.ОжидаетЧто(Ошибки)
			.ИмеетДлину(0);
	ЮТест.ОжидаетЧто(Предупреждения)
			.ИмеетДлинуБольше(0)
			.Содержит(ЮТест.Предикат() // Значение по условиям
    		.СодержитСтрокуПоШаблону("Свойство признано устаревшим! Необходимо отказаться от использования в кратчайшее время."))
КонецПроцедуры
...
Процедура Ошибка_ЧислоМеньшеМинимума_неВключая() Экспорт
	ОбъектПроверки = Новый Соответствие;
	ОбъектПроверки.Вставить("exclusiveminmax", 1);
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			.ИмеетДлинуБольше(0)
			.Содержит(ЮТест.Предикат() // Значение по условиям
    		.СодержитСтрокуПоШаблону("Значение <1> должно быть больше <1>"))
КонецПроцедуры

Процедура Ошибка_ЧислоБольшеМаксимума_неВключая() Экспорт
	ОбъектПроверки = Новый Соответствие;
	ОбъектПроверки.Вставить("exclusiveminmax", 10);
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			.ИмеетДлинуБольше(0)
			.Содержит(ЮТест.Предикат() // Значение по условиям
    		.СодержитСтрокуПоШаблону("Значение <10> должно быть меньше <10>"))
КонецПроцедуры
...
Процедура Успешно_ЧислоРавноМинимуму() Экспорт
	ОбъектПроверки = Новый Соответствие;
	ОбъектПроверки.Вставить("minmax", 1);
...
Процедура Успешно_ЧислоРавноМаксимуму() Экспорт
	ОбъектПроверки = Новый Соответствие;
	ОбъектПроверки.Вставить("minmax", 10);
...
Процедура Успешно_ФорматЦелогоЧисла(ИдентификаторФормата) Экспорт
	ОбъектПроверки = Новый Соответствие;
	ОбъектПроверки.Вставить(ИдентификаторФормата, 42);
...
Процедура Ошибка_НекорректныйФорматЦелого(ИдентификаторФормата, Значение) Экспорт
	ОбъектПроверки = Новый Соответствие;
	ОбъектПроверки.Вставить(ИдентификаторФормата, Значение);
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			.ИмеетДлинуБольше(0)
			.Содержит(ЮТест.Предикат() // Значение по условиям
    		.СодержитСтрокуПоШаблону("Некорректный формат значения, число должно быть \d+-битным целым."))
...
Процедура Ошибка_ПроверкаРаботыЗапретаДопСвойств() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			.ИмеетДлинуБольше(0)
			.Содержит(ЮТест.Предикат() // Значение по условиям
    		.СодержитСтрокуПоШаблону("Всегда запрещено."))
